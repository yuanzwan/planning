#include "an_static_planner/static_planner.h"
...
//fills obstacle information in the map
void Planner::generateMap() {
    for (auto& obs:obstacles_.obs) {
        int lane = static_cast<int>(obs.path[0].traj[0].position.y / kLaneWidth);
        int station = static_cast<int>(obs.path[0].traj[0].position.x / kStationLength);
        map_[lane][station] = true;
    }
}
...
bool Planner::checkCollision(double ego_x, double ego_y, double theta, an_messages::obstacle& obs) {
    double cos = std::cos(theta);
    double sin = std::sin(theta);
    double obs_x = obs.path[0].traj[0].position.x;
    double obs_y = obs.path[0].traj[0].position.y;
    ...
    for (int i = 0; i < motions_.size(); ++i) {
            ...
            if (lane >= 0 && lane < map_.size() && station < map_[0].size() && !map_[lane][station]) {
                    ...
                    for (auto& obs:obstacles_.obs) {
                            ...
                            if (obs.path[0].traj[0].position.x < kStationLength * node.station ||
                                    obs.path[0].traj[0].position.x > kStationLength * station) continue;
                            for (auto& traj:motions_[i].trajectory) {
                                    ...
                                    double r_square = (ego_x - obs.path[0].traj[0].position.x) * (ego_x - obs.path[0].traj[0].position.x)
                                            + (ego_y - obs.path[0].traj[0].position.y) * (ego_y - obs.path[0].traj[0].position.y);
                                    ...
                                    else if (checkCollision(ego_x, ego_y, traj.theta, obs)) {
                                            is_collision = true;
                                        break;
                                    }
                            }
...
void Planner::plan() {
    ...
    std::vector<std::vector<int>> closed(map_.size(), std::vector<int>(map_[0].size(), -1));
    auto cmp = [](Node& left, Node& right) {return left.f_value > right.f_value; };
    ...
    while (lane != start_lane || station != start_station) {
            auto& motion = motions_[closed[lane][station]];
    ...
    for (int i = 0; i < path.size(); ++i) {
        ...
        traj[i].header.stamp = obstacles_.obs[0].path[0].traj[0].header.stamp + ros::Duration(path[i][3] -  time_elapsed);
    }
    //publish trajectory
